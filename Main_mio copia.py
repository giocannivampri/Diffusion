...
def D_calculatore(I, delta=0.00000001):
    ...
    int_result = integrate.quad(
        (lambda th:
           scala* epsilon**2 * (np.sqrt(2*I)*np.cos(th) * f(np.sqrt(2*I)*np.sin(th), r_1, r_2) / (delta + np.sqrt(2*I)*np.sin(th)))**2),
        (np.arcsin(r_1/(delta + np.sqrt(2*I)))),
        np.pi / 2)
    ...
    return (int_result[0] * 0.5 * th_MAX**2 * r_2**2 * (beta/ emittance)/ (delta+(np.pi / 2)-(np.arcsin(r_1/(delta + np.sqrt(2*I)))))
...
azione=np.linspace(Min_I, Max_I, n_step)
...
dx.plot(times/dt*scala, current*len(x0)*dt/scala)

#Diffusion as a function of I
cx.set_xlabel("I")
cx.set_ylabel("D")
cx.plot(azione, diffusione/scala)
